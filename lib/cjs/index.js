...
const eventparsers_1 = require("./eventparsers");
    ...
    constructor() {
        ...
        this.digest = (rawEvent) => {
            if (!rawEvent)
                return null;
            let event = rawEvent;
            if (!(rawEvent.category in eventparsers_1.parsers)) {
                event = {
                    type: 'unknown',
            ...
            const newState = (0, utils_1.createStateCopy)(this.current);
            (0, eventparsers_1.parseCommonEventData)(event, newState);
            eventparsers_1.parsers[event.type](event, newState);
            const { leaderboardNames, sortedSquads } = (0, utils_1.calculateLeaderboards)(newState);
            ...
            (0, utils_1.fillExtensions)(newState.squads, this.teams, this.players);
