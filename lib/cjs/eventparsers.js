...
exports.parsers = exports.parseCommonEventData = void 0;
const utils_js_1 = require("./utils.js");
const ensureSquadExists = (state, squadName) => {
...
const ensurePlayerExists = (squad, playerName) => {
...
const constructPlayer = (squad, rawPlayer) => {
...
const updatePlayerSquad = (currentSquad, player, rawPlayer, state) => {
    if (currentSquad.name === rawPlayer.teamName)
        return;
...
const updatePlayerData = (squad, rawPlayer, state) => {
    ...
    if (player.type !== 'playing')
        return;
    player.distanceRun += (0, utils_js_1.calculateDistance)(player.pos, rawPlayer.pos);
...
const ensureRawPlayerExists = (state, rawPlayer) => {
...
const parseGameStateChanged = (event, state) => {
    state.gameState = event.state;
};
const parseMatchSetup = (event, state) => {
    state.server = {
...
const parseWeaponSwitchedPlayer = (event, state) => {
    const player = ensureRawPlayerExists(state, event.player);
    if (player && player.type === 'playing') {
        ensurePlayerHasWeapon(player, event.newWeapon);
    }
};
const parsePlayerDamaged = (event, state) => {
    ...
    if (attacker && attacker.type === 'playing') {
        attacker.damageLog.push({
            ...
            weapon: event.weapon,
            distance: (0, utils_js_1.calculateDistance)(event.attacker.pos, event.victim.pos),
            timestamp: state.currentTimestamp
        });
    }
    ensureRawPlayerExists(state, event.victim);
};
const parsePlayerStatChanged = (event, state) => {
    const player = ensureRawPlayerExists(state, event.player);
    if (player && player.type === 'playing') {
        switch (event.statName) {
            ...
            case 'assists':
                ...
                break;
            default:
                (0, utils_js_1.displayUnexpectedDataInfo)('player stat name', event.statName, event);
        }
    }
};
const parseInventoryDrop = (event, state) => {
    const player = ensureRawPlayerExists(state, event.player);
    if (player && player.type === 'playing') {
        let changed = false;
...
const parseGrenadeThrown = (event, state) => {
    const player = ensureRawPlayerExists(state, event.player);
    if (player && player.type === 'playing') {
...
const parseInventoryPickup = (event, state) => {
    const player = ensureRawPlayerExists(state, event.player);
    if (player && player.type === 'playing') {
        player.currentWeapons.push(event.item);
    }
};
const parseInventoryUse = (event, state) => {
    const player = ensureRawPlayerExists(state, event.player);
    if (player && player.type === 'playing') {
...
const parseMatchStateEnd = (event, state) => {
    state.gameState = event.state;
...
const parsePlayerAssist = (event, state) => {
    ensureRawPlayerExists(state, event.assistant);
    ensureRawPlayerExists(state, event.victim);
};
const parsePlayerKilled = (event, state) => {
    const attacker = ensureRawPlayerExists(state, event.attacker);
...
const parseSquadEliminated = (event, state) => {
...
const parseAmmoUsed = (event, state) => {
    const player = ensureRawPlayerExists(state, event.player);
    if (player && player.type === 'playing') {
...
const parsePlayerDowned = (event, state) => {
    const attacker = ensureRawPlayerExists(state, event.attacker);
    const victim = ensureRawPlayerExists(state, event.victim);
    if (attacker && attacker.type === 'playing' && victim) {
...
const parseRingFinishedClosing = (event, state) => {
    state.currentRing = {
        stage: event.stage,
        radius: event.currentRadius,
        center: event.center,
...
const parseWraithPortal = (event, state) => {
    ensureRawPlayerExists(state, event.player);
};
const parseZiplineUsed = (event, state) => {
    ensureRawPlayerExists(state, event.player);
};
const parseGibraltarShieldAbsorbed = (event, state) => {
    ensureRawPlayerExists(state, event.attacker);
    ensureRawPlayerExists(state, event.victim);
};
const parseBannerCollected = (event, state) => {
    ensureRawPlayerExists(state, event.player);
    ensureRawPlayerExists(state, event.collected);
};
const parseUnknownEvent = (event, _state) => {
    (0, utils_js_1.displayUnexpectedDataInfo)('event', event.category, event);
};
exports.parsers = {
    init: parseInit,
