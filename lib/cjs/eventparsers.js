...
const updatePlayerData = (squad, rawPlayer, state) => {
    ...
    if (player.type !== 'playing')
        return;
    player.distanceRun += (0, utils_1.calculateDistance)(player.pos, rawPlayer.pos);
...
const parsePlayerDamaged = (event, state) => {
    ...
    if (attacker && attacker.type === 'playing') {
        attacker.damageLog.push({
            ...
            weapon: event.weapon,
            distance: (0, utils_1.calculateDistance)(event.attacker.pos, event.victim.pos),
            timestamp: state.currentTimestamp
        });
    }
    ...
    if (player && player.type === 'playing') {
        switch (event.statName) {
            ...
            case 'assists':
                ...
                break;
            default:
                (0, utils_1.displayUnexpectedDataInfo)('player stat name', event.statName, event);
        }
    }
};
...
const parseUnknownEvent = (event, _state) => {
    (0, utils_1.displayUnexpectedDataInfo)('event', event.category, event);
};
exports.parsers = {
    init: parseInit,
